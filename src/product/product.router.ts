...
import type { ProductWrite } from "./product.service.js";
...
productRouter.post(
  ...
  async (request: Request, response: Response) => {
    ...
    try {
      const productData: Omit<ProductWrite, "id"> = request.body;
...
productRouter.put(
  ...
  async (request: Request, response: Response) => {
    ...
    const id: number = parseInt(request.params.id, 10);
    try {
      const updatedDataProduct: Omit<ProductWrite, "id"> = request.body;
