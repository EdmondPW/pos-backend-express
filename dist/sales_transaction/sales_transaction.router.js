...
salesTransactionRouter.get("/", authenticateUser, async (request, response) => {
    try {
...
salesTransactionRouter.get("/:id", authenticateUser, async (request, response) => {
...
salesTransactionRouter.get("/by-user-id/:user_id", authenticateUser, async (request, response) => {
    const id = parseInt(request.params.user_id, 10);
    try {
        const user = await SalesTransactionService.getOneSalesTransactionByUserId(id);
        if (user) {
            return response.status(200).json(user);
        }
        return response
...
salesTransactionRouter.post("/last-login/:user_id", authenticateUser, body("lastLogin").isDate(), async (request, response) => {
    const id = parseInt(request.params.user_id);
    const { lastLogin } = request.body;
    try {
        const user = await SalesTransactionService.getAllSalesTransactionAfterLastLoginDate(lastLogin, id);
        if (user) {
            return response.status(200).json(user);
        }
        return response
...
salesTransactionRouter.post("/", authenticateUser, body("sales_transaction_number").isString(), body("sales_transaction_status").isString(), body("total_price").isInt(), body("total_discount").isInt(), body("total_paid_cash").isInt(), body("total_paid_debit").isInt(), body("total_paid_credit").isInt(), body("total_paid_transfer").isInt(), body("total_paid_ojol").isInt(), body("total_nett").isInt(), body("cash_back").isInt(), body("user_id").isInt(), body("customer_type_id").isInt(), async (request, response) => {
    ...
    try {
        ...
        const newSalesTransaction = await SalesTransactionService.createSalesTransaction(salesTransactionData);
...
//       total_nett: true,
//       cash_back: true,
salesTransactionRouter.put("/:id", authenticateUser, body("sales_transaction_number").isString(), body("sales_transaction_status").isString(), body("total_price").isInt(), body("total_discount").isInt(), body("total_paid_cash").isInt(), body("total_paid_debit").isInt(), body("total_paid_credit").isInt(), body("total_paid_transfer").isInt(), body("total_paid_ojol").isInt(), body("total_nett").isInt(), body("cash_back").isInt(), body("user_id").isInt(), body("customer_type_id").isInt(), async (request, response) => {
    const errors = validationResult(request);
    if (!errors.isEmpty()) {
    ...
    try {
        ...
        const updatedSalesTransaction = await SalesTransactionService.updateSalesTransaction(updatedDataSalesTransaction, id);
...
salesTransactionRouter.delete("/:id", authenticateUser, async (request, response) => {
    const id = parseInt(request.params.id, 10);
    try {
